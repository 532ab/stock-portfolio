...
router.get("/", authMiddleware, async (req, res) => {
  ...
  try {
    const portfolio = await Portfolio.find({ userId: userId.toString() }); // assuming userId stored as string
...
router.post("/", authMiddleware, async (req, res) => {
  ...
  try {
    let stock = await Portfolio.findOne({ userId: userId.toString(), ticker: ticker.toUpperCase() });

    if (stock) {
      stock.quantity += quantity;
      await stock.save();
    } else {
      stock = new Portfolio({ userId: userId.toString(), ticker: ticker.toUpperCase(), quantity });
      await stock.save();
    }
...
router.delete("/:ticker", authMiddleware, async (req, res) => {
  ...
  try {
    await Portfolio.deleteOne({ userId: userId.toString(), ticker });
