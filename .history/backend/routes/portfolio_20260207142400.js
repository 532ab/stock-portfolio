      ...
      portfolio.map(async (stock) => {
        try {
          ...
          const finnhubResponse = await axios.get(
            `https://finnhub.io/api/v1/quote?symbol=${stock.ticker}&token=d1t74k9r01qh0t057ov0d1t74k9r01qh0t057ovg`,
            { timeout: 5000 }
          );
...
router.post("/", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    try {
      // Try Finnhub first
      const finnhubResponse = await axios.get(
        `https://finnhub.io/api/v1/quote?symbol=${ticker.toUpperCase()}&token=d1t74k9r01qh0t057ov0d1t74k9r01qh0t057ovg`,
        { timeout: 5000 }
      );
